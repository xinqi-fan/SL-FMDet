...
def get_annotation(annotation_name):   # (num_obj, 5)
    ...
    for object in annotation_tree.findall('object'):
        ...
        # otherwise annotation[0, -1]   remains 0 as background
...
def convert_corner2center(boxes, cla):
    ...
    # return np.concatenate((((boxes[:, 2:] + boxes[:, :2]) / 2).astype(np.int),  # cx, cy
...
def main():
    # dir
    dataset_root = '../../Data_Bank/Face_Detection/Moxa3K'
    image_path = dataset_root + '/images'
    annotation_path = dataset_root + '/annotations/Pascal Voc'
    save_path = dataset_root + '/heatmap_add_mobilenet_big/'
    ...
    # iterate each image
    target_size = (80, 80) # mobilenet0.25
    ...
    for index in range(num_sample):
        ...
        # resize image
        # output_img = cv2.resize(heatmap, target_size) * 255
        output_img = heatmap * 255
        # output_img.dtype='uint8'
